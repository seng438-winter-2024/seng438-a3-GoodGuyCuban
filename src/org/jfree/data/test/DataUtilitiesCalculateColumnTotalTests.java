...
public class DataUtilitiesCalculateColumnTotalTests extends DataUtilities {
    ...
    @Test
    public void calculateColumnTotalWithNullAndNumericValues() {
        // setup
        Mockery mockingContext = new Mockery();
        final Values2D values = mockingContext.mock(Values2D.class);
        mockingContext.checking(new Expectations() {{
            ...
            will(returnValue(5));
            one(values).getValue(1, 0);
            will(returnValue(null)); // Simulate a non-numeric value with null
            one(values).getValue(2, 0);
            will(returnValue(10));
        }});
        double result = DataUtilities.calculateColumnTotal(values, 0);
        ...
        // verify
        assertEquals("Sum was unexpected.", 15, result, .000000001d);
    }
